 ...
 * 
 * This webhook is triggered when a shop requests deletion of their data (e.g., app uninstall).
 * 
 * Requirements:
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      // Only log non-sensitive metadata (no shop-specific sensitive data)
      const payloadSize = bodyBuffer.length
      console.log(`[Webhook ${timestamp}] Payload parsed`, {
        topic,
        payloadSize,
        hasShopData: !!payload?.shop_id,
      })
    } catch (error) {
