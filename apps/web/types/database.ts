...
export interface Site {
  ...
  api_key: string | null;
  // HMAC-SHA256 secret the WP plugin uses to sign outbound webhooks.
  // Added in migration 00005. Nullable for legacy rows; new rows get a
  // fresh secret generated by createSite. Webhook delivery for sites
  // with a NULL secret is a silent no-op on the receiver side.
...
export interface Page {
  ...
  title: string;
  url: string;
  // WordPress post_status string ("publish", "draft", "private", etc.).
  // Added in migration 00004 to support status filtering in the cache.
  status: string;
  last_updated: string;
